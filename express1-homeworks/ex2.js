...
app.get('/:category/:subcategory/:id' , (req,res) => {
  ...
  if(foundCategory){
const foundSubcategory = foundCategory.subcategories.find(sub => sub.subcategory === subcategory) 
  if(foundSubcategory){

const foundProducts = foundSubcategory.products.find(pord => pord.id === parseInt(id))
if(foundProducts){
res.send(foundProducts)
return;
}else{
  res.send('did not find the product')
  return;
}
}
else{
  res.send('did not find the subcategory')
  return;
}
  }
else{
  res.status(404).send('did not find the category')
  return;
}
